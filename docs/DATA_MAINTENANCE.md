...
`catalog.txt` 继续保存真实静态分类。侧边栏顺序直接采用一级标题的源文件顺序，并通过 `@ui-section main|emoji|other|hidden` 决定显示区域，不再维护容易失配的标题白名单。
...
14 个 Emoji 分类在数据中是独立一级分类，界面显示在 `Emoji` 父级下面。日文、韩文、东亚字符、俄文和古文字同样是一级分类，显示在“其他符号”父级下面。`补充符号`保留为内部数据分类，不显示独立侧边栏入口。
...
JSON只由构建工具读取。`yesymbol.exe` 运行时不解析JSON，而是直接读取编译进EXE的静态C数组。`tests\static_check.py` 会核对JSON符号数量、生成的C计数、来源位置数组长度和典型符号的分类/行列位置。
